...
CREATE TABLE VOLUNTARIO(
    ...
    CONSTRAINT CK_VOLUNTARIO_CPF CHECK (REGEXP_LIKE(CPF, '\d\d\d.\d\d\d.\d\d\d-\d\d')),
    CONSTRAINT CK_VOLUNTARIO_TEL CHECK (REGEXP_LIKE(TELEFONE, '\(\d\d\) \d\d\d\d\d-\d\d\d\d'))
...
CREATE TABLE VOLUNTARIO_AMADOR(
    ...
    CONSTRAINT FK_VOLUNTARIO_AMADOR FOREIGN KEY (VOLUNTARIO)
                                  REFERENCES VOLUNTARIO(CPF)
                                  ON DELETE CASCADE,
    CONSTRAINT CK_VOLUNTARIO_AMADOR CHECK (REGEXP_LIKE(CEP, '\d\d\d\d\d-\d\d\d'))
);
...
CREATE TABLE VOLUNTARIO_PROFISSIONAL(
    ...
    CONSTRAINT FK_VOLUNTARIO_PROFISSIONAL FOREIGN KEY (VOLUNTARIO)
                                  REFERENCES VOLUNTARIO(CPF)
                                  ON DELETE CASCADE,
    CONSTRAINT CK_VOLUNTARIO_PROFISSIONAL CHECK (REGEXP_LIKE(CRM, '\w\w\w/\w\w \d\d\d\d\d\d'))
);
...
CREATE TABLE PRODUTO_DOADO(
    VOLUNTARIO CHAR(14) NOT NULL,
    DATA DATE NOT NULL,
    TIPO VARCHAR2(30) NOT NULL, -- VERIFICAR
...
CREATE TABLE EVENTO (
    ...
    CONSTRAINT CK_EVENTO CHECK (REGEXP_LIKE(CEP, '\d\d\d\d\d-\d\d\d'))
);
...
CREATE TABLE MEMBRO(
    ...
    CONSTRAINT CK_MEMBRO_CPF CHECK (REGEXP_LIKE(CPF, '\d\d\d.\d\d\d.\d\d\d-\d\d')),
    CONSTRAINT CK_MEMBRO_TEL CHECK (REGEXP_LIKE(TELEFONE, '\(\d\d\) \d\d\d\d\d-\d\d\d\d')),
    CONSTRAINT CK_MEMBRO_CEP CHECK (REGEXP_LIKE(CEP, '\d\d\d\d\d-\d\d\d'))
);
...
CREATE TABLE PETSHOP(
    ...
    CONSTRAINT CK_PETSHOP_CEP CHECK (REGEXP_LIKE(CEP, '\d\d\d\d\d-\d\d\d')),
    CONSTRAINT CK_PETSHOP_CNPJ CHECK (REGEXP_LIKE(CNPJ, '\d\d.\d\d\d.\d\d\d/\d\d\d\d-\d\d'))
);
