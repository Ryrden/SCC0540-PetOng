...
CREATE TABLE VOLUNTARIO_AMADOR(
    ...
    CONSTRAINT CK_VOLUNTARIO_AMADOR_NUMERO CHECK (NUMERO > 0)
);
...
-- Não é uma boa ter um Check para Valor > 0?
CREATE TABLE DINHEIRO_ARRECADADO(
    ...
    CONSTRAINT FK_DINHEIRO_ARRECADADO FOREIGN KEY (VOLUNTARIO)
                                      ...
                                      ON DELETE CASCADE,
    CONSTRAINT CK_DINHEIRO_ARRECADADO_VALOR CHECK (VALOR > 0)
);

-- Não é uma boa ter um Check para Quantidade > 0?
CREATE TABLE PRODUTO_DOADO(
    ...
    CONSTRAINT CK3_PRODUTO_DOADO_QUANTIDADE CHECK (QUANTIDADE > 0)
);
...
CREATE TABLE EVENTO (
    ...
    CONSTRAINT CK_EVENTO_NUMERO CHECK (NUMERO > 0)
);
...
CREATE TABLE MEMBRO(
    ...
    CONSTRAINT CK_MEMBRO_NUMERO CHECK (NUMERO > 0)
);
...
-- Não é uma boa ter um Check para numero de pets > 0?
CREATE TABLE PETSHOP(
    ...
    CONSTRAINT CK_PETSHOP_NUMERO CHECK (NUMERO > 0),
    CONSTRAINT CK_PETSHOP_N_PETS CHECK (N_PETS > 0)
);
...
CREATE TABLE PET(
    ...
    CONSTRAINT FK2_PET FOREIGN KEY(DONO)
                       REFERENCES VOLUNTARIO_AMADOR(VOLUNTARIO)
                       ON DELETE SET NULL,
    CONSTRAINT CK_PET_IDADE CHECK (IDADE > 0)
);

-- atributo Vacina deve ser chamar Vacina_Contra
...
-- Não sei se é importante mas a ordem que é definida a Primary Key importa?
-- Falta uma FK para petshop
